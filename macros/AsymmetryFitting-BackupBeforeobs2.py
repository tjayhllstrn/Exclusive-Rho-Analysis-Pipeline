...
def main(inputFiles,outputDir,
    ...
    results = []
    legend_names = []
    for bn_idx in range(len(cthbin)-1): 
        if fit_type[0]=="1":
            results.append(RunMhFitMLM(obs,bin_edges,inputFiles,outputDir,treeName,cthbin[bn_idx],cthbin[bn_idx+1]))
            legend_names.append(f"A_{{sig}} MLM Fit to M_{{h}} cos#theta ({cthbin[bn_idx]},{cthbin[bn_idx+1])}")
        if fit_type[1]=="1":
            results.append(RunMhFitChi2(bin_edges,phibn_edges,inputFiles,outputDir,obs,treeName,cthbin[bn_idx],cthbin[bn_idx+1]))
            legend_names.append(f"A_{{sig}} Chi2 Fit to M_{{h}} cos#theta ({cthbin[bn_idx]},{cthbin[bn_idx+1]})")
        if fit_type[2]=="1":
            results.append(RunMxFitMLM(obs,bin_edges,inputFiles,outputDir,treeName,cthbin[bn_idx],cthbin[bn_idx+1]))
            legend_names.append(f"A_{{sig}} MLM Fit to M_{{x}} cos#theta ({cthbin[bn_idx]},{cthbin[bn_idx+1]})")
        if fit_type[3]=="1":
            results.append(RunMxFitChi2(bin_edges,phibn_edges,inputFiles,outputDir,obs,treeName,cthbin[bn_idx],cthbin[bn_idx+1]))
            legend_names.append(f"A_{{sig}} Chi2 Fit to M_{{x}} cos#theta ({cthbin[bn_idx]},{cthbin[bn_idx+1]})")
...
def RunMxFitMLM(obs,bin_edges,inputFiles,outputDir,treeName,cthmin,cthmax):
    sigbkg_ub = 1.15
    sigbkg_lb = 0.85
    bkg_ub = 2.75
    bkg_lb = 1.4
    ...
    for idx in range(len(bin_edges)-1):
        ...
        u_results.append(purityCalc_mxFit(inputFiles,outputDir,"MxMLM",sigbkg_lb,sigbkg_ub,obs,obsmin,obsmax,cthmin,cthmax,treeName,True))
    ...
    PurityOutputFile = os.path.join(outputDir,f"PurityFits/Mx_purityPerBin_cth{cthmin}-{cthmax}")
...
def RunMxFitChi2(bin_edges,phibn_edges,inputFiles,outputDir,obs,treeName,cthmin,cthmax):
    ...
    alpha = runner.Run_mxFit(bin_edges,phibn_edges,inputFiles,outputDir,obs,treeName,cthmin,cthmax)
...
def RunMhFitMLM(obs,bin_edges,inputFiles,outputDir,treeName,cthmin,cthmax):
    sigbkg_ub = 0.9
    sigbkg_lb = 0.65
    bkg_ub = 1.7
    bkg_lb = 1.1
    ...
    for idx in range(len(bin_edges)-1):
        obsmin = bin_edges[idx]
        obsmax = bin_edges[idx+1]
        u_results.append(purityCalc(inputFiles,outputDir,"MhMLM",sigbkg_lb,sigbkg_ub,obs,obsmin,obsmax,cthmin,cthmax,treeName,True))
    ...
    PurityOutputFile = os.path.join(outputDir,f"PurityFits/Mh_purityPerBin_cth{cthmin}-{cthmax}")
    outroot = ROOT.TFile(f"{PurityOutputFile}.root","RECREATE")
    c = ROOT.TCanvas()
    gr.Draw("AP")
    leg.Draw("SAME")
...
def RunMhFitChi2(bin_edges,phibn_edges,inputFiles,outputDir,obs,treeName,cthmin,cthmax):
    ROOT.gSystem.Load("/w/hallb-scshelf2102/clas12/users/tjhellst/clas-ana-scaffold-tyler/macros/libPhiBinnedFitRunner.so")
    runner = ROOT.PhiBinnedFitRunner()
    alpha = runner.Run(bin_edges,phibn_edges,inputFiles,outputDir,obs,treeName,cthmin,cthmax)            
    results = list(alpha) 
    A_sig_results = []
    for i in range(0,len(results),2):
        A_sig_results.append((results[i],results[i+1]))
