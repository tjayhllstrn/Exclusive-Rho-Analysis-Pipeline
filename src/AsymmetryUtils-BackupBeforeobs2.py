...
def RunFittingMLM(obs,bin_edges,inputFiles,treeName,lower_bound,upper_bound,cthmin,cthmax,exclusiveFit,fit_type="sim"):
    ...
    if exclusiveFit == "Mh":
        if treeName == "pippi0":
            cut_str = f"Mdiphoton<0.16 && 0.115<Mdiphoton && 0.85<Mx && Mx < 1.05 && {lower_bound}<Mh && Mh<{upper_bound} && {cthmin}<cth&&cth<{cthmax}"
        elif treeName == "pippim":
            cut_str = f"0.85<Mx && Mx < 1.05 && {lower_bound}<Mh && Mh<{upper_bound}&& {cthmin}<cth&&cth<{cthmax}"
    elif exclusiveFit == "Mx":
        if treeName == "pippi0":
            cut_str = f"Mdiphoton<0.16 && 0.115<Mdiphoton && 0.65<Mh&&Mh<0.9 && {lower_bound}<Mx && Mx<{upper_bound}&& {cthmin}<cth&&cth<{cthmax}"
        elif treeName == "pippim":
            cut_str = f"0.65<Mh&&Mh<0.9 && {lower_bound}<Mx && Mx<{upper_bound}&& {cthmin}<cth&&cth<{cthmax}"
...
def purityCalc(inputFiles, outputDir,filename,lb,ub,obs_str,obsmin,obsmax,cthmin,cthmax,treeName,save_purity_plot=True):
    #chain data:
    ...
    cth = RooRealVar("cth","cth",-1,1)
    obs = RooRealVar(f"{obs_str}",f"{obs_str}",obsmin,obsmax)
    if treeName == "pippi0":
        Mdiphoton = RooRealVar("Mdiphoton","Mdiphoton",0.115,0.16)
        cut_str = f"Mdiphoton<0.16 && 0.115<Mdiphoton && 0.85<Mx && Mx < 1.05 && {obs_str}>{obsmin} && {obs_str}<{obsmax}&& {cthmin}<cth&&cth<{cthmax}"
        roo_DS = RooDataSet("roo_DS","roo_DS",chain,ROOT.RooArgSet(Mh,Mdiphoton,Mx,obs,cth),cut_str)
    else:
        cut_str = f"0.85<Mx && Mx < 1.05 && {obs_str}>{obsmin} && {obs_str}<{obsmax}&& {cthmin}<cth&&cth<{cthmax}"
        roo_DS = RooDataSet("roo_DS","roo_DS",chain,ROOT.RooArgSet(Mh,Mx,obs,cth),cut_str)
    ...
    if save_purity_plot:
        plotname = f"{filename}_PurityFit_z{round(obsmin,2)}-{round(obsmax,2)}_cth{round(cthmin,2)}-{round(cthmax,2)}"
...
def purityCalc_mxFit(inputFiles, outputDir,filename,lb,ub,obs_str,obsmin,obsmax,cthmin,cthmax,treeName,save_purity_plot=True):
    #chain data:
    chain = ROOT.TChain(treeName)
    for f in inputFiles:
        chain.Add(f)
    ...
    if treeName == "pippi0":
        Mdiphoton = RooRealVar("Mdiphoton","Mdiphoton",0.115,0.16)
        cut_str = f"Mdiphoton<0.16 && 0.115<Mdiphoton && 0.65<Mh&&Mh<0.9 && {obs_str}>{obsmin} && {obs_str}<{obsmax}&& {cthmin}<cth&&cth<{cthmax}"
        roo_DS = RooDataSet("roo_DS","roo_DS",chain,ROOT.RooArgSet(Mx,Mdiphoton,Mh,obs,cth),cut_str)
    else:
        cut_str = f"0.65<Mh&&Mh<0.9 && {obs_str}>{obsmin} && {obs_str}<{obsmax}&& {cthmin}<cth&&cth<{cthmax}"
        roo_DS = RooDataSet("roo_DS","roo_DS",chain,ROOT.RooArgSet(Mx,Mh,obs,cth),cut_str)
